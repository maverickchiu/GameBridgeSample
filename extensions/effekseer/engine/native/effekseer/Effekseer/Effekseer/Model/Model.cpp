...
Model::Model(const void* data, int32_t size)
{
	...
	for (int32_t f = 0; f < frameCount; f++)
	{
        ...
        auto target = models_[f].vertexes.data();
		...
		if (version_ >= 1)
		{
            for (int32_t i = 0; i < vertexCount; i++) {
                auto rv = (VertexR*)p;
                auto& wv = target[i];
                wv.VColor = rv->VColor.ToFloat4();

				memcpy(target + i, p, sizeof(VertexR) - sizeof(Color));
                p += sizeof(VertexR);
            }
		}
		else
		{
            for (int32_t i = 0; i < vertexCount; i++) {
                memcpy(target + i, p, sizeof(Vertex) - sizeof(Color4F));
                p += sizeof(Vertex) - sizeof(Color4F);

                auto& wv = target[i];
                wv.VColor = {1.0, 1.0, 1.0, 1.0};
            }
		}
...
bool Model::StoreBufferToGPU(Backend::GraphicsDevice* graphicsDevice, bool flipVertexColor)
{
	...
	for (int32_t f = 0; f < GetFrameCount(); f++)
	{
		if (flipVertexColor)
		{
			...
			for (auto& v : vdata)
			{
				std::swap(v.VColor[0], v.VColor[2]);
			}
